<template>
  <div class="actions--contain">
    <div class="actions">
      ...
      <div class="actions__action-editor" v-if="editorOn">
...
export default {
  ...
  watch: {
    ...
    currentActionID(n) {
      ...
      this.$nextTick().then(() => (this.edited = false));
    },
  },
  methods: {
    ...
    editorSave() {
      ...
      this.client.actions.splice(this.currentActionID, 1, newAction);

      this.edited = false;
    },
    moveAction(index) {
      const newPosition =
        this.client.actions.length - +prompt('На какую позицию переместить?');
    ...
    copyAction(index) {
      const action = JSON.parse(JSON.stringify(this.actions[index]));
    ...
    resetChanges() {
      this.client.actions = JSON.parse(
        JSON.stringify(this.client.oldData)
      ).actions;
    },
    saveChanges() {
      this.SAVE_USER_DATA(JSON.parse(JSON.stringify(this.client)));
      this.client.oldData = JSON.parse(
        JSON.stringify({
          ...this.client.oldData,
...
.actions {
  ...
  &__table {
    display: grid;
    grid-gap: padding();
    grid-template-columns: $id-cs repeat(4, 1fr);
